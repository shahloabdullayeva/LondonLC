...
function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"results" | "students" | "teachers" | "practice" | "tests" | "writing" | "articles" | "music" | "profile">("profile");
  ...
  const [allSubmissions, setAllSubmissions] = useState<(WritingSubmission & { studentName: string })[]>([]);
  ...
  const refreshData = async (teacherId?: string) => {
    const [all, teacherList, studentList, ips, prReqs, subs] = await Promise.all([
      getAttempts(), getTeachers(), getStudentAccounts(), getBlockedIPs(), getPremiumRequests(), getAllSubmissions(),
    ]);
    setAttempts(all);
  ...
  useEffect(() => {
    ...
    if (tab === "tests") {
      setActiveTab("tests");
      if (type === "reading" || type === "listening") { setTestsTypeFilter(type); setTestsSelectedBook(null); }
    } else if (tab === "writing") {
  ...
  const writingRows: AttemptData[] = allSubmissions
    .filter((w) => !teacherIdSet.has(w.studentId))
    .map((w) => {
      const acc = studentById.get(w.studentId);
      return {
        id: w.id, studentId: w.studentId,
        studentName: acc?.name ?? w.studentName ?? "Unknown",
        studentSurname: acc?.surname ?? "",
        groupName: acc?.group_name ?? "—",
        testId: "", testTitle: w.prompt.length > 60 ? w.prompt.slice(0, 60) + "…" : w.prompt,
        testType: "writing" as const, testLevel: "academic" as const,
        answers: {}, score: 0, maxScore: 0, bandScore: w.overallBand ?? 0,
        status: "completed" as const, startedAt: w.createdAt,
        submittedAt: w.gradedAt ?? w.createdAt, timeSpentSeconds: 0, isTeacherAttempt: false,
      };
    });
  const allRows = [...attempts, ...writingRows];
  const submissionById = new Map(allSubmissions.map((s) => [s.id, s]));

  const groups = ["all", ...Array.from(new Set(allRows.map((a) => a.groupName)))];
  ...
  const uniqueStudents = new Set(allRows.filter((a) => !a.isTeacherAttempt).map((a) => a.studentId)).size;
  ...
  const canManageTeachers = isRootAdmin;
  ...
  return (
    <StudentShell wide>
      <div style={{ color: C.text }}>
        ...
        {/* Persistent admin sub-nav — stays visible across Profile / Results /
            Students / Manage Teachers so the shortcuts never disappear. */}
        {(activeTab === "profile" || activeTab === "results" || activeTab === "students" || activeTab === "teachers") && (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 24 }}>
            {([
              { id: "profile" as const, Icon: UserCircle2, label: "My Profile", show: true, badge: undefined as number | undefined },
              { id: "results" as const, Icon: BarChart3, label: "Results", show: true, badge: undefined as number | undefined },
              { id: "students" as const, Icon: Users, label: "Students", show: true, badge: premiumRequests.filter(r => r.status === "pending").length || undefined },
              { id: "teachers" as const, Icon: Shield, label: "Manage Teachers", show: canManageTeachers, badge: undefined as number | undefined },
            ]).filter(c => c.show).map(({ id, Icon, label, badge }) => (
              <button key={id} onClick={() => setActiveTab(id)}
                style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "9px 16px", borderRadius: 10, border: `1px solid ${activeTab === id ? C.accent : C.border}`, background: activeTab === id ? C.accentLight : "transparent", color: activeTab === id ? C.accent : C.muted, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                <Icon size={15} /> {label}
                {badge ? <span style={{ background: "#ef4444", color: "#fff", fontSize: 10, fontWeight: 800, padding: "1px 6px", borderRadius: 999 }}>{badge}</span> : null}
              </button>
            ))}
          </div>
        )}
        ...
        {activeTab === "results" && <>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20, marginBottom: 24, paddingBottom: 20, borderBottom: `1px solid ${C.border}`, flexWrap: "wrap" }}>
            <div style={{ flexShrink: 0 }}>
              ...
              <p style={{ fontSize: 13, color: C.muted }}>Signed in as <strong style={{ color: isRootAdmin ? "#fbbf24" : isAdminUser ? "#60a5fa" : C.accent }}>{currentUsername}</strong></p>
            </div>
          ...
          {filtered.length > 0 ? (
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, overflow: "hidden", marginBottom: 28, maxHeight: "calc(100vh - 250px)", display: "flex", flexDirection: "column" }}>
                    ...
                    {filtered.map((a, i) => {
                      ...
                      return (
                        ...
                        <tr style={{ borderBottom: `1px solid ${C.border}`, background: i % 2 === 0 ? C.card : "rgba(6,12,31,0.6)" }}>
                          ...
                          <td style={{ padding: "12px 14px", color: C.sub }}>{a.testType === "writing" || a.testType === "speaking" ? "–" : `${a.score}/${a.maxScore}`}</td>
                        ...
                        {isExpanded && (
                          <tr key={`${a.id}-detail`} style={{ background: "var(--site-border)" }}>
                            <td colSpan={10} style={{ padding: "16px 20px", borderBottom: `1px solid ${C.border}` }}>
                              {a.testType === "writing" && submissionById.get(a.id) && (
              ...
              <div style={{ background: C.card, border: `1px solid rgba(239,68,68,0.2)`, borderRadius: 12, overflow: "hidden" }}>
                {blockedIPs.map(ip => (
                  <div key={ip} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", borderBottom: `1px solid ${C.border}` }}>
                    <span style={{ fontFamily: "ui-monospace, SFMono-Regular, Menlo, Consolas, monospace", fontSize: 13, color: C.text }}>{ip}</span>
        ...
        {activeTab === "students" && (
          <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
            <TabHeader title="Student Accounts" subtitle="" C={C} />
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    {[{ label: "Username", val: createdStudent.username }, { label: "Password", val: createdStudent.password }].map(item => (
                      <div key={item.label} style={{ padding: "10px 14px", background: "var(--site-border)", borderRadius: 9, border: `1px solid ${C.border}` }}>
                        ...
                        <div style={{ fontFamily: "ui-monospace, SFMono-Regular, Menlo, Consolas, monospace", fontSize: 15, fontWeight: 700, color: C.text }}>{item.val}</div>
                      </div>
                    ))}
                  </div>
                    ...
                    <tbody>
                      {students.filter(s => !studentSearch || `${s.name} ${s.surname} ${s.username} ${s.group_name}`.toLowerCase().includes(studentSearch.toLowerCase())).map((s, i) => (
                        ...
                        <tr style={{ background: i % 2 === 0 ? "transparent" : "var(--site-border)", borderBottom: editingStudentId === s.id ? "none" : `1px solid ${C.border}` }}>
                          ...
                          <td style={{ padding: "12px 14px", fontFamily: "ui-monospace, SFMono-Regular, Menlo, Consolas, monospace", fontSize: 13, color: C.sub }}>{s.username}</td>
                          <td style={{ padding: "12px 14px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              {resetResult?.id === s.id ? (
                                <>
                                  <span style={{ fontFamily: "ui-monospace, SFMono-Regular, Menlo, Consolas, monospace", fontSize: 13, color: C.accent }}>{resetResult.password}</span>
                                ...
                                <>
                                  <span style={{ fontFamily: "ui-monospace, SFMono-Regular, Menlo, Consolas, monospace", fontSize: 13, color: C.muted }}>••••••••</span>
                              ...
                              <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                                {[
                                  ...
                                  <div key={item.label} style={{ padding: "8px 14px", background: C.card2, borderRadius: 8, border: `1px solid ${C.border}` }}>
                                    ...
                                    <div style={{ fontSize: 13, color: C.sub, fontFamily: "ui-monospace, SFMono-Regular, Menlo, Consolas, monospace" }}>{item.val}</div>
                                  </div>
                                ))}
        ...
        {activeTab === "tests" && (() => {
          ...
          return (
            <div>
              <TabHeader title="Tests" subtitle="" C={C} />
        ...
        {activeTab === "profile" && (() => {
          ...
          return (
            <div style={{ maxWidth: 640 }}>
              <TabHeader title="My Profile" subtitle="" C={C} />
        ...
        {activeTab === "writing" && (
          <div>
            <TabHeader title="Writing Submissions" subtitle="" C={C} />
        ...
        {activeTab === "teachers" && canManageTeachers && (
          <div style={{ maxWidth: 900 }}>
            <TabHeader title="Manage Teachers" subtitle="" C={C} />
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div>
                        ...
                        {resetResult?.id === t.id && !isProtected && (
                          <div style={{ fontSize: 12, marginTop: 2, fontFamily: "ui-monospace, SFMono-Regular, Menlo, Consolas, monospace", color: C.accent, display: "flex", alignItems: "center", gap: 6 }}>
...
function TabHeader({ title, subtitle, C }: {
  title: string;
  subtitle: string;
    ...
    <div style={{
      ...
      <div style={{ flexShrink: 0 }}>
        <h1 style={{
          ...
          color: C.text, marginBottom: 0, lineHeight: 1.15,
        }}>{title}</h1>
