...
import { getSession, saveSpeakingResult, saveAttempt, type StudentSession } from "@/lib/store";
...
async function speak(text: string): Promise<void> {
  ...
  const missionControl = process.env.NEXT_PUBLIC_MISSION_CONTROL;

  if (missionControl) {
    try {
      const res = await fetch(`${missionControl}/functions/v1/speak-question`, {
        method: "POST",
...
function SpeakingPage() {
  ...
  const [convoQuestions, setConvoQuestions] = useState<string[]>([]);
  const recognitionRef = useRef</* SpeechRecognition */ unknown>(null);

  useEffect(() => {
    const s = getSession();
    if (!s) { router.push("/auth/login"); return; }
    if (s.isAdmin && !isPractice) { router.push("/admin/dashboard"); return; }
    setSession(s);
  }, [router, isPractice]);
  ...
  const currentQ = convoQuestions[qIdx];
  ...
  const getFollowUp = useCallback(async (part: number, history: { role: string; text: string }[]): Promise<string> => {
    const missionControl = process.env.NEXT_PUBLIC_MISSION_CONTROL;
    if (!missionControl) return "";
    try {
      const res = await fetch(`${missionControl}/functions/v1/speaking-turn`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ part, history }),
      });
      if (!res.ok) return "";
  ...
  const nextQuestion = useCallback(async () => {
    ...
    const q = currentQ;
    let history = answers.flatMap(a => [
      { role: "examiner", text: a.question },
      { role: "student", text: a.transcript },
    ]);
    if (part && q && transcript) {
      setAnswers(prev => [...prev, { part: part.part, question: q, transcript }]);
      history = [...history, { role: "examiner", text: q }, { role: "student", text: transcript }];
    }
    setLiveTranscript("");

    if (!selectedTest || !part) return;

    const advancePart = () => {
      if (partIdx < selectedTest.parts.length - 1) {
        const nextIdx = partIdx + 1;
        setPartIdx(nextIdx);
        setQIdx(0);
        setConvoQuestions([selectedTest.parts[nextIdx].questions[0]]);
        setPhase("intro");
      } else {
        gradeAnswers();
      }
    ...
    const maxPerPart = part.part === 2 ? 1 : 5;
    const answeredInPart = qIdx + 1;
    if (part.part === 2 || answeredInPart >= maxPerPart) {
      advancePart();
      return;
    }

    setPhase("speaking");
    let nextQ = await getFollowUp(part.part, history);
    if (!nextQ) nextQ = part.questions[answeredInPart] ?? "";
    if (!nextQ) { advancePart(); return; }

    setConvoQuestions(prev => [...prev, nextQ]);
    setQIdx(answeredInPart);
    setIsSpeaking(true);
    await speak(nextQ);
    setIsSpeaking(false);
    startRecording();
  }, [liveTranscript, currentPart, currentQ, answers, selectedTest, partIdx, qIdx, getFollowUp, startRecording]);
  ...
  const gradeAnswers = async () => {
    ...
    try {
      const res = await fetch(`${missionControl}/functions/v1/grade-speaking`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        const data = await res.json();
        setResult(data);
        if (session && selectedTest && typeof data.overall === "number") {
          const now = new Date().toISOString();
          saveSpeakingResult(session.id, {
            id: `sp-${Date.now()}`, title: selectedTest.title,
            overall: data.overall, createdAt: now,
          });
          saveAttempt({
            id: `spk-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
            studentId: session.id, studentName: session.name, studentSurname: session.surname,
            groupName: session.group_name, testId: selectedTest.id, testTitle: selectedTest.title,
            testType: "speaking", testLevel: "academic", answers: {}, score: 0, maxScore: 0,
            bandScore: data.overall, status: "completed", startedAt: now, submittedAt: now,
            timeSpentSeconds: 0, isTeacherAttempt: isPractice,
            teacherId: isPractice ? session.id : undefined,
          });
        }
      }
    } catch {}
  ...
  const startTest = (test: typeof TESTS[0]) => {
    ...
    setPartIdx(0);
    setQIdx(0);
    setConvoQuestions([test.parts[0].questions[0]]);
    setAnswers([]);
    setResult(null);
    setPhase("intro");
  };
  ...
  if (phase === "select") {
    return (
      <StudentShell>
        <h1 className="h1"><em>Speaking</em></h1>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 14, marginTop: 28 }}>
          {TESTS.map(test => (
            <div key={test.id} className="card" style={{ padding: 20, cursor: "pointer", transition: "border-color 0.15s, transform 0.15s" }}
              ...
              <h3 style={{ fontFamily: DISPLAY, fontSize: 18, fontWeight: 500, marginBottom: 16 }}>{test.title}</h3>
  ...
  if (phase === "results") {
    return (
      <StudentShell>
        <h1 className="h1"><em>Score</em></h1>
        {result ? (
